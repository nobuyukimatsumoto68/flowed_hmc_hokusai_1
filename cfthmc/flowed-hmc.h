...
qacc void set_xl_nu_from_mask_mu_yl_m_plaq(Coordinate& xl, int& nu,
                                           ...
                                           const Geometry& geo, const int flow_type)
...
qacc void set_xg_nu_from_mask_mu_yg_m_srect_nu_neq_mu(
    Coordinate& xg, int& nu, const int mask_yl, const int nu_size, const int mask,
    const int mu, const Coordinate& yg, const int m, const int flow_type)
{
  qassert(flow_type == 2);
...
// Normally call this at the end of Flowed-HMC evolution step.
{
  ...
  for (int i = 0; i < (int)fi.v.size(); ++i) {
    const FlowStepInfo& fsi = fi.v[i];
    refresh_expanded_gf_flow_plaq_mask_mu(gf_ext, fsi.mask, fsi.mu, fsi.flow_size);
...
// Normally call this at the beginning of Flowed-HMC evolution step.
{
  ...
  for (int i = (int)fi.v.size() - 1; i >= 0; --i) {
    const FlowStepInfo& fsi = fi.v[i];
    refresh_expanded_gf_flow_plaq_mask_mu(gf_ext, fsi.mask, fsi.mu, fsi.flow_size);
    gf_flow_inv_plaq_mask_mu_no_comm(gf_ext, gf_ext, fsi.mask, fsi.mu, fsi.epsilon,
                                   fsi.flow_size);
  }
  gf.init(geo);
  gf = gf_ext;
...
// mu: flow link direction
{
  ...
  qacc_for(index, geo.local_volume(), {
    ...
    for (int m = 0; m < multiplicity_flow_hmc(mask_yl == mask, flow_size); ++m) {
      Coordinate xl;
      int nu;
...
inline void set_mp_mat_plaq_mask_mu_no_comm(FieldM<AdjointColorMatrix, 1>& mpf,
...
//
{
  ...
  for (int i = 0; i < (int)fi.v.size(); ++i) {
    ...
    refresh_expanded_gf_flow_plaq_mask_mu(gf_ext, fsi.mask, fsi.mu, fsi.flow_size);
    FieldM<ColorMatrix, 1> cf;
    set_flow_staple_mask_mu_no_comm(cf, gf_ext, fsi.mask, fsi.mu, fsi.flow_size);
    FieldM<AdjointColorMatrix, 1> nf;
    set_n_mat_plaq_mask_mu_no_comm(nf, cf, gf_ext, fsi.mask, fsi.mu, fsi.flow_size);
...
// All gauge fields are refreshed except gf_ext_vec.back().
{
  ...
  for (int i = 0; i < (int)fi.v.size(); ++i) {
    ...
    gf_flow_plaq_mask_mu_no_comm(gf_ext_vec[i + 1], gf_ext_vec[i], fsi.mask, fsi.mu,
                               fsi.epsilon, fsi.flow_size);
  }
}
